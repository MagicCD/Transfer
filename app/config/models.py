...
class BaseConfig(BaseModel):
    ...
    def validate_dependencies(self) -> 'BaseConfig':
        """验证配置项之间的依赖关系"""
        # 验证分块大小必须小于或等于分块上传阈值
        if self.CHUNK_SIZE > self.CHUNKED_UPLOAD_THRESHOLD:
            raise ValueError("分块大小必须小于或等于分块上传阈值")
        
        # 验证分块上传阈值必须小于或等于最大内容长度
        if self.CHUNKED_UPLOAD_THRESHOLD > self.MAX_CONTENT_LENGTH:
